...
import { getLightColor, getDarkColor, isClient, isFunction } from "@mist/helper";
...
const mtBgColorElm = "--mt-bg-color-elm";
const mtBgColorMute = "--mt-bg-color-mute";
...
export const useThemeColor = (color: MaybeRef<string>, ignoreList?: string[] | (() => string[] | undefined)) => {
  ...
  const switchLight = () => {
    ...
    const lightVarSpreadMap = {
      ...
      [mtBgColorElm]: getLightColor(primary, 0.945)!,
      [mtBgColorMute]: getLightColor(primary, 0.91)!,
    };
  ...
  const switchDark = () => {
    ...
    const darkVarSpreadMap = {
      ...
      [mtBgColorElm]: getDarkColor(primary, 0.92)!,
      [mtBgColorMute]: getDarkColor(primary, 0.91)!,
    };
