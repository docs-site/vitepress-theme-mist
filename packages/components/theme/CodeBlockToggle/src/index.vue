...
import type { CodeBlock } from "@mist/config";
import { nextTick, watch } from "vue";
import { useEventListener, useNamespace } from "@mist/composables";
import { addUnit, isBoolean, isClient } from "@mist/helper";
import { arrowDownIcon } from "@mist/static";
...
const codeBlockConfig = getMistConfigRef<CodeBlock>("codeBlock", {
  enabled: true,
  collapseHeight: 700,
...
const initCodeBlock = () => {
  ...
  Array.from(modes).forEach(item => {
    ...
    copyDom?.addEventListener("click", () => {
      codeBlockConfig.value.copiedDone?.(MtMessage);
    });

    // 忽略部分 class：代码块父元素的 class 中包含 details、mt-vp-code 则跳过
