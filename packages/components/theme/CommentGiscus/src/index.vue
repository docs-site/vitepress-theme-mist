...
import type { CommentProvider } from "@mist/config";
import { ref, nextTick, onMounted, computed, inject } from "vue";
import { useData } from "vitepress";
import { isFunction } from "@mist/helper";
import { useNamespace, useVpRouter } from "@mist/composables";
...
const { getMistConfig } = useMistConfig();
const { isDark } = useData();

const giscusOptions = getMistConfig<CommentProvider["giscus"]>("comment", {}).options;
...
onMounted(() => {
  if (!useOnline && !giscusComponent) {
    return console.error(
      "[Mist Error] Giscus initialization failed. Please configure the 'useOnline' to 'true' or provide the giscus component"
    );
  }
  reloadGiscus();
  // 路由切换后更新评论内容
