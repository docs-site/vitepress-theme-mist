...
import { useLocale, useMediaQuery, useNamespace } from "@mist/composables";
import { autoWidthIcon } from "@mist/static";
...
<template>
  <slot v-if="!isLt960" :active :icon="autoWidthIcon" :toggleSideBar>
    <div
      ...
      @click="toggleSideBar"
      :title="t('mt.sidebarTrigger.label')"
      role="button"
      :aria-label="t('mt.sidebarTrigger.label')"
    >
      <div :class="[ns.join('right-bottom-button__button')]">
        <MtIcon :icon="autoWidthIcon" aria-hidden="true" />
      </div>
    </div>
  </slot>
</template>
