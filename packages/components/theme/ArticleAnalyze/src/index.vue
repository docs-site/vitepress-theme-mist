...
import type { ArticleAnalyze, DocAnalysis, DocDocAnalysisFileInfo, MistConfig } from "@mist/config";
import type { MtContentData } from "@mist/config";
import { computed, nextTick, ref, watch } from "vue";
import { useData } from "vitepress";
import { useNamespace, useLocale, useUvPv, useVpRouter } from "@mist/composables";
import { readingIcon, clockIcon, viewIcon } from "@mist/static/icons";
...
const { getMistConfig, getMistConfigRef } = useMistConfig();
const { theme, frontmatter } = useData();
const vpRouter = useVpRouter();
const { router } = vpRouter;

// 文章基本信息
const post = computed<MtContentData>(() => ({
  author: getMistConfig<MistConfig["author"]>("author", {}),
...
// 文章信息配置项
const articleConfig = getMistConfigRef<ArticleAnalyze>("articleAnalyze", {
  showInfo: true,
  showIcon: true,
  teleport: {},
});
...
const docAnalysisConfig = getMistConfigRef<DocAnalysis>("docAnalysis", {
  wordCount: true,
  readingTime: true,
  statistics: {},
});
...
<template>
  <div :class="`${ns.b()} flx-justify-between`" :aria-label="t('mt.articleAnalyze.label')">
    <MtArticleBreadcrumb />

    <div v-if="isShowInfo" ref="baseInfoRef" :class="`${ns.e('wrapper')} flx-align-center`">
      <MtArticleInfo :post scope="article" />

      <div v-if="docAnalysisConfig.wordCount || pageViewInfo.wordCount" class="flx-center">
        <MtIcon v-if="articleConfig.showIcon" :icon="readingIcon" aria-hidden="true" />
        <a :title="t('mt.articleAnalyze.wordCount')" class="hover-color" :aria-label="t('mt.articleAnalyze.wordCount')">
          {{ pageViewInfo.wordCount }}
        </a>
      </div>

      <div v-if="docAnalysisConfig.readingTime || pageViewInfo.readingTime" class="flx-center">
        <MtIcon v-if="articleConfig.showIcon" :icon="clockIcon" />
        <a
          :title="t('mt.articleAnalyze.readingTime')"
          class="hover-color"
          :aria-label="t('mt.articleAnalyze.readingTime')"
        >
      ...
      <div v-if="usePageView" class="flx-center">
        <MtIcon v-if="articleConfig.showIcon" :icon="viewIcon" />
        <a :title="t('mt.articleAnalyze.pageView')" class="hover-color" :aria-label="t('mt.articleAnalyze.pageView')">
          {{ isGet ? pagePv : "Get..." }}
        </a>
      </div>
    </div>
