...
import type { ArticleAnalyze } from "@mist/config";
import { computed } from "vue";
import { useEventListener } from "@mist/composables";
import { useMistConfig } from "@mist/components/theme/ConfigProvider";
import { createImageViewer } from "@mist/components/common/ImageViewer";

const { getMistConfig } = useMistConfig();

// 文章图片配置
const imageViewer = computed(() => {
  const { imageViewer = {} } = getMistConfig<ArticleAnalyze>("articleAnalyze", {});
  return imageViewer;
});
