...
// 生成section ID（用于跳转）
const generateSectionId = (title: string) => title.replace(/[^a-zA-Z0-9\u4e00-\u9fa5]/g, "-").toLowerCase();
...
onMounted(() => {
  ...
  const handleScroll = () => {
    ...
    const currentSection = sections.reduce((prev, current) => {
      ...
      return current.top < viewportHeight * 0.5 && currentDistance < prevDistance ? current : prev;
    }, sections[0]);
      ...
      <main :class="ns.e('content')">
        ...
        >
          ...
          <div :class="ns.e('links')">
            <NavigationPageLinkItem v-for="item in section.items" :key="item.title" v-bind="item" />
          </div>
        </div>
