...
import type { ArticleAnalyze, ArticleInfoPosition, MtContentData } from "@mist/config";
import { useRoute, withBase, useData } from "vitepress";
import { computed } from "vue";
import { formatDate, isFunction } from "@mist/helper";
import { useNamespace, useLocale } from "@mist/composables";
import { userIcon, calendarIcon, editPenIcon, folderOpenedIcon, collectionTagIcon } from "@mist/static";
import { useMistConfig, usePosts } from "@mist/components/theme/ConfigProvider";
...
<template>
  <div :class="[ns.b(), scope]" role="group" :aria-label="t('mt.articleInfo.label')">
    <template v-for="item in baseInfo" :key="item.title">
      ...
      >
        <MtIcon v-if="articleConfig.showIcon" :icon="item.icon" :class="ns.e('icon')" aria-hidden="true" />
        <a
          v-if="item.data"
          :title="item.title"
