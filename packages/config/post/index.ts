import type { FileContentLoaderData } from "@docs-site/vitepress-plugin-file-content-loader";
import type { MtContentData, PostData } from "./types";
import type { RequiredKeyPartialOther } from "@mist/helper";
import type { SiteConfig } from "vitepress";

import { basename, join } from "node:path";
import { statSync } from "node:fs";
import { formatDate } from "@mist/helper";
import {
  filterPosts,
 ...
 * 转换为文章数据
 */
export const transformData = (data: FileContentLoaderData): MtContentData => {
 ...
 * 转换为各个文章不同类型的数据
 */
export const transformRaw = (posts: MtContentData[]): PostData => {
  const siteConfig: SiteConfig = (globalThis as any).VITEPRESS_CONFIG;
...
const resolvePosts = (posts: MtContentData[]): PostData => {
 ...
 * @param post 文章数据
 */
export function getTitle(post: RequiredKeyPartialOther<MtContentData, "frontmatter" | "relativePath">) {
 ...
 * @param srcDir 项目绝对路径
 */
export function getDate(post: RequiredKeyPartialOther<MtContentData, "frontmatter" | "relativePath">, srcDir: string) {
 ...
 */
export const getCaptureText = (post: MtContentData, count = 300) => {
  const { content = "" } = matter(post.src || "", {});
