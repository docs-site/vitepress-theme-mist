import type { GroupCardItem, MtContentData, PostData } from "./types";
 ...
 * @param posts 所有文章数据
 */
export const filterPosts = (posts: MtContentData[]): MtContentData[] => {
 ...
 */
export const getSortPostsByDateAndSticky = (posts: MtContentData[]): MtContentData[] => {
 ...
 * @param posts 过滤非文章页之后的文章数据
 */
export const getSortPostsByDate = (posts: MtContentData[]): MtContentData[] => {
  // sort 会改变原数组，因此确保参数不被修改，复杂一份
  const p = [...posts];
 ...
 */
export const getGroupPosts = (posts: MtContentData[]): PostData["groupPosts"] => {
  const categoriesObj: Record<string, MtContentData[]> = {};
  const tagsObj: Record<string, MtContentData[]> = {};
 ...
 * @param post 文章数据
 */
export const getPostsTime = (post: MtContentData): number => {
 ...
 * @param next 文章 2
 */
export const compareDate = (prev: MtContentData, next: MtContentData) => {
 ...
 * @param posts 文章列表
 */
export const groupByYear = (posts: MtContentData[]) => {
  return posts.reduce(
    (pre, cur) => {
 ...
 * @param posts 文章列表
 */
export const groupByYearMonth = (posts: MtContentData[]) => {
  return posts.reduce(
    (pre, cur) => {
