...
```bash
# 推荐使用 pnpm
pnpm i @docs-site/vitepress-plugin-permalink
# or yarn
...
`createRewrites` 函数支持除了传入 `@docs-site/vitepress-plugin-permalink` 的 [配置项](https://github.com/Kele-Bingtang/vitepress-theme-teek/blob/master/plugins/@docs-site/vitepress-plugin-permalink/src/types.ts)，也支持额外传入两个配置项：
